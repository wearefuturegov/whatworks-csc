...
RSpec.describe PersonDecorator, :vcr do
  ...
  let(:slug) { 'person' }
  let(:subject) { Person.find_by(slug: slug).load.first.decorate }
  
  describe '#decorate_image' do
        
    context 'with image' do
      
      before { stub_api_response(slug, 'person_with_image') }
    ...
    context 'without image' do
      ...
      before { stub_api_response(slug, 'person_without_image') }
